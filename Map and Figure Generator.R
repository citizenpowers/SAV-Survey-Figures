...
for(i in seq_along(Region_names[[3]]))   
{
  ...
  assign("work_area",Region_names[[i,3]])   #creates a variable from the cell name 
  assign("region",Region_names[[i,1]])   #creates a variable from the region name
  
  #Sum of Coverages (Abundances) stacked columns 
  plot_abundance <-ggplot(veg_abundance_and_frequency(All_SAV_Data,work_area),aes(reorder(as.character(Date,format="%Y %b"),Date),Abundance,fill=SPECIES))+geom_col(position = "stack")+
  ...
  axis.line = element_line(colour = "black"),plot.title =element_text(size=18,face="bold",hjust=.2),aspect.ratio = 2/3)+
  labs(x="Date",y="Frequency of SAV Occurrence",title=paste("SAV Frequency by Species in",work_area))
  
  ggsave(paste("Figures/",work_area,"_Frequency.jpg"),plot=plot_frequency, width = 9, height = 6, units = "in") #save frequency plot
  
  #Frequency of Occurance Plot with total SAV frequency
  plot_frequency_with_total_SAV <- ggplot(veg_abundance_and_frequency(All_SAV_Data,work_area),aes(reorder(as.character(Date,format="%Y %b %d"),Date),Frequency,fill=SPECIES))+geom_col(position = "stack")+
  ...
  axis.line = element_line(colour = "black"),plot.title =element_text(size=18,face="bold",hjust=.2))+
  labs(x="Date",y="Frequency of SAV Occurrence",title=paste("SAV Frequency by Species in",work_area))
  
  ggsave(paste("Figures/",work_area,"_Frequency_with_total_SAV.jpg"),plot=plot_frequency_with_total_SAV) #save frequency plot
  
  #Relative SAV Coverage by Species plot
  plot_Relative_SAV_Coverage <-ggplot(veg_abundance_and_frequency(All_SAV_Data,work_area),aes(reorder(as.character(Date,format="%Y %b"),Date),`Relative SAV Coverage`,fill=SPECIES))+geom_col(position = "stack")+
  scale_fill_brewer(type="qual",palette = "Spectral",direction=-1,name ="Species",breaks=c("CERATOPHYLLUM", "CHARA", "HYDRILLA", "NAJAS_GUADALUPENSIS", "NAJAS_MARINA", "POTAMOGETON", "UTRICULARIA", "VALLISNERIA"),
  labels=c("Ceratophyllum", "Chara", "Hydrilla", "Najas guadalupensis", "Najas marina", "Potamogeton", "Utricularia", "Vallisneria"))+
  labs(x="Survey Date",y="Relative SAV Coverage (%)",title=paste("Relative SAV Coverage by Species in ",work_area))+
  ...
  ggsave(paste("Figures/",work_area,"_SAV_Over_Time.jpg"),plot=map_codominant_veg) #save dominant vegetation map
